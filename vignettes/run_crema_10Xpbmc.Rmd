...
The preprocessing R code of the dataset is in `data_human_10Xpbmc.Rmd` in the [`CREMA_manuscript` repository](https://github.com/zidongzh/CREMA_manuscript). Note that we recommend creating a new `atac_inserts.tsv.gz` file from the original `atac_fragments.tsv.gz` file provided by 10X Genomics. See below for detailed explanation.

```{r}
data_multi <- readRDS(file.path("../../data_seurat_object/10X_pbmc/multiome.rds"))
```
...
```{r}
frag_inserts_object <- CreateFragmentObject(path = file.path("../../data/10X_pbmc/multiome", "pbmc_granulocyte_sorted_10k_atac_inserts.tsv.gz"), 
